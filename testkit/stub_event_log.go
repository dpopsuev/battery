...
var _ event.EventLog = (*StubEventLog)(nil)

// StubEventLog is an in-memory EventLog for testing.
// Thread-safe, records all events, supports OnEmit callbacks.
...
func (l *StubEventLog) Emit(e event.Event) int {
	l.mu.Lock()
	defer l.mu.Unlock()
...
func (l *StubEventLog) Since(index int) []event.Event {
	l.mu.Lock()
	defer l.mu.Unlock()
	...
	out := make([]event.Event, len(l.events)-index)
...
func (l *StubEventLog) OnEmit(fn func(event.Event)) {
	l.mu.Lock()
	defer l.mu.Unlock()
